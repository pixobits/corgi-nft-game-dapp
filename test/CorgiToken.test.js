const CorgiToken = artifacts.require("./CorgiToken.sol");

require("chai")
  .use(require("chai-as-promised"))
  .should();

contract("Corgi Token", (accounts) => {
  let token;
  describe("deployment", async () => {
    it("deploys successfully", async () => {
      token = await CorgiToken.deployed();
    ...
    it("has a name", async () => {
      ...
      assert.equal(name, "Corgi Token");
    });

    it("has a symbol", async () => {
      const symbol = await token.symbol();
      assert.equal(symbol, "CORGI");
    });
  });
